...
class QuantumConnection(Connection):
    def __init__(self, length, name='QuantumConnection', direction='A2B', models=None, p_loss_init=0, p_loss_length=0.2):
        ...
        if direction != 'B2A':
            self.add_subcomponent(QuantumChannel('QChannel_A2B', length=length,delay=length/2e5, models=models),
                                  forward_input=[('A', 'send')],
                                  forward_output=[('B', 'recv')])

        if direction != 'A2B':
            self.add_subcomponent(QuantumChannel('QChannel_B2A', length=length, delay=length/2e5, models=models),
                                  forward_input=[('B', 'send')],
                                  forward_output=[('A', 'recv')])
# %%
